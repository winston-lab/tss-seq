...
call_de_clusters = function(intable, norm, libcounts, samples, groups, condition, control, alpha, results, normcounts, rldcounts, qcplots){
    #import data 
    ...
    dds = DESeqDataSetFromMatrix(countData = countdata,
                                 colData = coldata,
                                 design = ~ condition)
    #use original base counts for sizeFactor estimation since the cluster set
    #is biased to being differentially expressed
    sicountdata = get_countdata(libcounts, samples)
    ...
    sfdf = dds %>% sizeFactors() %>% as_tibble() %>%
            ...
            inner_join(colSums(sicountdata) %>% as_tibble() %>%
...
qc = call_de_clusters(intable = snakemake@input[["clustercounts"]],
                      norm = snakemake@wildcards[["norm"]],
                      libcounts = snakemake@input[["libcounts"]],
