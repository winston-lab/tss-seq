        ...
        if (logtxn){
            heatmap_base = heatmap_base +
                ...
                scale_fill_viridis(option = cmap, na.value="#FFFFFF00",
                                   name= if (strand != "both"){bquote(bold(log[2] ~ .(strand) ~ "TSS-seq" ~ signal))} else {bquote(bold(log[2] ~ "TSS-seq" ~ signal))},
...
main(in_paths = snakemake@input[["matrices"]],
     ...
     cluster_samples = c("depleted-1-antisense", "depleted-2-antisense"),
