...
rule all:
    input:
        ...
        expand(expand("motifs/{condition}-v-{control}/spikenorm/{{category}}/{condition}-v-{control}_tss-seq-spikenorm-diffexp-results-{{category}}-{{direction}}-meme_chip/summary.tsv", zip, condition=conditiongroups_si, control=controlgroups_si), category="intragenic", direction=["up"]) if config["motifs"]["meme-chip"]["run-meme-chip"] and comparisons_si else [],
