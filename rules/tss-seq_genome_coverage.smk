...
rule normalize_genome_coverage:
    ...
    shell: """
        awk -v total_read_count=$(grep -oh "\w*{params.prefix}\w*" {input.chrsizes} | xargs samtools view -c {input.bam} | paste -d "" - <(echo "*{params.scale_factor}/1000000") | bc -l) '{{$4=$4/total_read_count; print $0}}' {input.counts} > {output.normalized}
        """
