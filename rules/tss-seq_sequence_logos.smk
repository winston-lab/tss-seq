...
rule get_seqlogo_data:
    ...
    run:
        ...
        shell("""(samtools merge - {input.bam} | bedtools bamtobed -i stdin | awk 'BEGIN{{FS=OFS="\t"}} $6=="+"{{print $1, $2, $2+1, $4, $5, $6}} $6=="-"{{print $1, $3-1, $3, $4, $5, $6}}' | {params.intersect_cmd} bedtools slop -b {params.slop} -s -i stdin -g <(faidx {input.fasta} -i chromsizes) | awk '$3-$2=={params.windowsize}' | bedtools getfasta -s -fi {input.fasta} -bed stdin | weblogo --format logodata --sequence-type dna --composition "{composition}" > {output.seqlogo_data}) &> {log}""")
